...
**現在地**: Phase 1 - Realtime ツール連携と VAD タイムアウト実装（55%完了）
**次のアクション**: 実オーディオ入力とのバインド（WakeWord → RealtimeSession）と Google API 実装
...
- [ ] Google Calendar API（本番）のクレデンシャル未設定のため、現在は in-memory 実装のみ
...
- [ ] `src/calendar/google_calendar_client.py` の CRUD 実装＋Push/ポーリング両対応（現状はメモリ実装）
