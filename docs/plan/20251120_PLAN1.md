...
**現在地**: Phase 2 - カレンダー連携・通知（完了）
**次のアクション**: Phase 3 - Raspberry Pi 最適化
**ブロッカー**: 音声再生未成功（音声データは受信しているがスピーカーから出力されない）／Raspberry Pi 実機テスト未実施
**重要な決定事項**: `src/calendar` を `src/gcal` にリネーム（標準モジュールとの競合回避）。予定削除・一覧取得ツールを追加。通知ループを実装。

### フェーズ進捗
- [x] Phase 0: 要件定義レビュー 完了
- [x] Phase 1: 音声/Realtime 土台（完了）
- [x] Phase 2: カレンダー連携・通知（完了）
...
| 日時 | 変更内容 | 関連ファイル |
|------|----------|---------------|
| 11-21 21:20 | Phase 2完了: カレンダー削除・一覧機能追加、通知ループ実装、`src/gcal`リネーム | `src/gcal/*`, `src/realtime/*`, `src/session/manager.py` |
...
- [x] `src/calendar/google_calendar_client.py` の CRUD 実装＋Push/ポーリング両対応（`src/gcal`へリネーム済み）
- [x] `src/util/reminder_scheduler.py`（新規）で10分前通知キュー管理
- [x] Realtime Session に Function Calling で返る JSON を Calendar Client へ橋渡し（API 呼び出し）
- [x] テスト: 偽イベントで10分前通知フロー（音声通知テキストのみ）
