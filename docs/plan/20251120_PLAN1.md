...
**現在地**: Phase 1 - Realtime & Calendar API スタブ拡張（80%完了）
**次のアクション**: Realtime WebSocket 送受信を実装し、Wake Word 後の対話レスポンス結合テスト（Calendar 呼び出し含む）
**ブロッカー**: API トークン未注入／Raspberry Pi の入出力デバイス検証待ち／Realtime 送信経路が未実装
...
### ブロッカー・未解決の問題
- [ ] RealtimeSession の音声送信/応答受信が未実装（Wake 後無反応の根因）
...
| `src/realtime/openai_realtime_client.py` | 🔄 作業中 | RealtimeSession ツール処理/VAD連携 | 会話制御 | Phase 1 |
