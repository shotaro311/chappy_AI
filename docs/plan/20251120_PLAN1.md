...
**現在地**: Phase 1 - Realtime & Calendar API スタブ拡張（65%完了）
**次のアクション**: WakeWord ↔ RealtimeSession の結線と Google API 実呼び出しの結合テスト
**ブロッカー**: API トークン未注入／Raspberry Pi の入出力デバイス検証待ち
**重要な決定事項**: Picovoice Porcupine + ChatGPT Realtime API + Google Calendar API の3本構成継続、開発ブランチ `feat/wake-gpt-plan` + Draft PR #1 基軸。RealtimeSession はツール呼び出しを Pydantic スキーマで受ける方針。

### フェーズ進捗
- [x] Phase 0: 要件定義レビュー 完了
- [🔄] Phase 1: 音声/Realtime 土台（55%）
...
| `src/util/reminder_scheduler.py` | ✅ 先行着手 | 10分前通知ロジックの下拵え | 通知制御 | Phase 2 |
| `src/calendar/google_calendar_client.py` | 🔄 着手 | Google API 対応＋メモリフォールバック | カレンダー連携 | Phase 2 |
