...
**バージョン**: 1.5
**ステータス**: ドラフト
**前バージョンからの変更**: Porcupine初期化問題解決＋実機起動成功（ウェイクワード待機状態）
...
**現在地**: Phase 1 - Realtime & Calendar API 拡張（85%完了）
**次のアクション**: OPENAI_API_KEY を設定し、実機で Realtime 音声送受信＋Calendar 結合テストを実施
**ブロッカー**: API トークン未注入／Raspberry Pi の入出力デバイス検証待ち／実機での応答確認未実施
**重要な決定事項**: Picovoice Porcupine + ChatGPT Realtime API + Google Calendar API の3本構成継続、開発ブランチ `feat/wake-gpt-plan` + Draft PR #1 基軸。RealtimeSession はツール呼び出しを Pydantic スキーマで受ける方針。全テスト（6件）成功。

### フェーズ進捗
- [x] Phase 0: 要件定義レビュー 完了
- [🔄] Phase 1: 音声/Realtime 土台（80%）
...
- ⏳ 次のタスク: Realtime WebSocket 送受信（音声送信＋応答受信）を実装し、Wake Word 後の対話レスポンスと Google API の結合テストを実施

### ブロッカー・未解決の問題
- [ ] 実機での Realtime 応答確認未実施（OPENAI_API_KEY 未注入）
- [ ] `webrtcvad` 導入済みだが実機で無音自動終了を未検証
...
| 11-20 23:58 | WakeWord をビルトインキーワード利用で起動可能に（porcupineデフォルト/ENV可） | `src/main.py`, `src/wakeword/porcupine_listener.py` |
| 11-20 23:55 | main.py に WakeWord待機→RealtimeSession 結線（フェイルセーフ含む） | `src/main.py`
