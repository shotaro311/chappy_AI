...
**バージョン**: 1.6
**ステータス**: ドラフト
**前バージョンからの変更**: Realtime API 部分動作確認 - 音声送信・テキスト応答受信成功、音声再生は未解決
...
**現在地**: Phase 1 - Realtime & Calendar API 拡張（90%完了）
**次のアクション**: 音声出力デバイス問題の解決（音声データ受信済みだが再生されず）
**ブロッカー**: 音声再生未成功（音声データは受信しているがスピーカーから出力されない）／Raspberry Pi 実機テスト未実施
**重要な決定事項**: Picovoice Porcupine + ChatGPT Realtime API + Google Calendar API の3本構成継続、開発ブランチ `feat/wake-gpt-plan` + Draft PR #1 基軸。RealtimeSession はツール呼び出しを Pydantic スキーマで受ける方針。Realtime API 音声送信・テキスト応答受信は動作確認済み（音声再生のみ未解決）。全テスト（6件）成功。

### フェーズ進捗
- [x] Phase 0: 要件定義レビュー 完了
- [🔄] Phase 1: 音声/Realtime 土台（90%）
- [ ] Phase 2: カレンダー連携・通知（0%）
- [ ] Phase 3: Raspberry Pi 最適化（0%）
- [ ] Phase 4: 統合テスト/運用化（0%）

### 環境チェックリスト
- [x] `OPENAI_API_KEY`（pc.dev）設定済み・動作確認済み
...
**最終更新**: 2025-11-21 02:10 JST
**進捗率**: 90%

### 直前の作業
- ✅ 完了: Realtime API 音声送信・テキスト応答受信の動作確認（ChatGPT: 「はい、聞こえていますよ。しっかりクリアに聞こえています。何か手伝えることがあれば、遠慮なく言ってください。」）
- ✅ 完了: Server VAD（2秒無音検出）による発話終了判定の動作確認
- ✅ 完了: `response.audio_transcript.delta` イベントのリアルタイムログ表示実装（`AI: ...`形式）
- ✅ 完了: 音声出力24kHz対応実装（`AudioOutputStream` にサンプルレート指定パラメータ追加）
- ✅ 完了: 音声データ受信確認（`response.audio.delta` イベント大量受信）
- ⚠️ 未解決: 音声再生が動作せず（音声データは受信しているがスピーカーから出力されない）
- ✅ 完了: RealtimeSession に WebSocket 送受信・server_vad セッション更新・応答生成トリガーを実装。音声出力はデバイスが無い環境では自動で無効化
- ✅ 完了: RealtimeSession をオフライン実行できる `connect=False` フラグを追加し、ユニットテストを固定化
- ✅ 完了: pc.dev 環境のデフォルト入出力デバイスを None 想定に揃え、`tests/config/test_loader.py` を修正（pytest 6/6 パス）
- ✅ 完了: wake loop ごとに Porcupine を再初期化し、マイク設定ログ出力を追加（WakeWord安定化）
- ✅ 完了: Realtime 送信用に 16k→24kHz リサンプルを実装（OpenAI Realtime 仕様に合わせるため）
...
- ✅ 完了: `src/realtime/schema.py` + `RealtimeSession` のツール呼び出し処理、VADタイムアウト、非同期 audio loop scaffolding
- ✅ 完了: `src/calendar/google_calendar_client.py` を Google API 対応 (intl fallback) へ更新
- ✅ 完了: `src/util/reminder_scheduler.py` とテスト（Phase2 の先行着手）
- ✅ 完了: pytest エラー全解消（6/6 テスト成功）
  - `_ensure_timezone()` 関数追加で NameError 解消
  - `SessionManager` に audio close 処理追加
  - `list_upcoming()` に `reference_time` パラメータ追加でテスト安定化
- ✅ 完了: `src/main.py` で WakeWord 待機→SessionManager→RealtimeSession のループを実装（Porcupine未設定時は Wake Word スキップ、VADは利用可能時のみ）
- ✅ 完了: `.env` ファイルの読み込み問題を解決（`override=True` で既存環境変数を上書き）
- ✅ 完了: Porcupine 日本語モデルファイル（`porcupine_params_ja.pv`）を GitHub から取得・配置
- ✅ 完了: オーディオデバイス設定をシステムデフォルトに変更（`config/pc.dev.yaml`）
- ✅ 完了: Porcupine の感度を 0.7 に設定し、デバッグログを追加（ウェイクワード検出率向上）
- ✅ 完了: 実機での起動テスト成功（ウェイクワード待機状態まで到達）
- ⏳ 次のタスク: 音声出力デバイス問題の解決（出力デバイス設定確認・デバッグ）

### ブロッカー・未解決の問題
- [ ] **音声再生未成功**（音声データ受信済みだがスピーカーから出力されない）- 明日対応予定
  - 可能性: 出力デバイス不一致（WH-1000XM4がデフォルトだが装着していない？）
  - デバッグ済み: 音声データ（response.audio.delta）は正常に受信している
- [x] 実機での Realtime 応答確認完了（テキスト応答受信成功）
- [ ] `webrtcvad` 導入済みだが実機で無音自動終了を未検証（Server VADは動作確認済み）
- [ ] Raspberry Pi 実機でのマイク/スピーカー型番未確定
- [x] Picovoice Porcupine の keyword / model ファイル配置完了（日本語キーワード「ちゃっぴい」+ `porcupine_params_ja.pv`）
- [ ] Google Calendar API（本番）のクレデンシャル未設定のため、現在は in-memory 実装のみ（コード上は対応済み）
- [ ] ウェイクワード検出の実機テストが未完了（感度調整が必要な可能性あり）

### 最近の変更履歴（直近5件）
| 日時 | 変更内容 | 関連ファイル |
|------|----------|---------------|
| 11-21 02:10 | Realtime API 音声送信・テキスト応答受信確認、音声再生は未解決 | `src/realtime/openai_realtime_client.py`, `src/audio/output_stream.py`, `config/base.yaml` |
| 11-21 01:35 | RealtimeSession に WebSocket 実装・オフラインフラグ追加、テスト維持 | `src/realtime/openai_realtime_client.py`, `tests/realtime/test_openai_realtime_session.py` |
| 11-21 01:05 | pc.dev デフォルト入出力を None 想定に揃えテスト修正、pytest 6/6 パス | `tests/config/test_loader.py` |
| 11-21 00:20 | Porcupine初期化問題解決＋感度0.7設定＋実機起動成功 | `src/config/loader.py`, `src/wakeword/porcupine_listener.py`, `config/pc.dev.yaml` |
| 11-21 00:02 | JA .ppn 利用時にモデル未配置なら明示エラーを出すよう修正 | `src/main.py`, `src/wakeword/porcupine_listener.py` |
...
| `src/config/loader.py` | ✅ 完了 | base+env マージ処理/Pydantic 型定義/.env明示読み込み | 設定読み込み | Phase 1 |
| `src/audio/input_stream.py`, `src/audio/output_stream.py` | ✅ 完了 | 音声 I/O 抽象クラス | 音声基盤 | Phase 1 |
| `src/realtime/openai_realtime_client.py` | 🔄 作業中 | WebSocket 送受信・server_vad 設定・オフライン実行フラグを追加 | 会話制御 | Phase 1 |
| `src/realtime/schema.py` | ✅ 完了 | Function Calling の Pydantic モデル | Realtime ツール連携 | Phase 1 |
| `src/util/reminder_scheduler.py` | ✅ 完了 | 10分前通知ロジック＋テスト安定化 | 通知制御 | Phase 2 |
| `src/calendar/google_calendar_client.py` | ✅ 完了 | Google API 対応＋timezone処理＋reference_time対応 | カレンダー連携 | Phase 1,2 |
| `src/session/manager.py` | ✅ 完了 | audio close 処理追加 | セッション管理 | Phase 1 |
| `src/wakeword/porcupine_listener.py` | ✅ 完了 | 感度0.7設定＋デバッグログ追加＋日本語モデル対応 | ウェイクワード検出 | Phase 1 |
| `config/pc.dev.yaml` | ✅ 完了 | オーディオデバイスをシステムデフォルトに変更 | 開発環境設定 | Phase 1 |
| `src/main.py` | ✅ 完了 | WakeWord待機→SessionManager→RealtimeSession ループ化（フェイルセーフあり） | エントリーポイント共通化 | Phase 1 |
| `tests/**` | ✅ 完了 | pc.dev 入出力 None 想定で config テスト修正＋全6テスト成功 | テスト安定化 | Phase 1 |
...
- [🔄] `src/calendar/google_calendar_client.py` の CRUD 実装＋Push/ポーリング両対応（API クライアント土台は実装済み・Push/Watch未)
- [x] `src/util/reminder_scheduler.py`（新規）で10分前通知キュー管理
- [ ] Realtime Session に Function Calling で返る JSON を Calendar Client へ橋渡し（API 呼び出し）
